...
class ScreenCapture:
    ...
    def calibrate_region(self, display_duration=3.0):
        ...
        self.start()
        time.sleep(1)  # Give time for thread to capture first frame
    ...
    def get_monitor_region(self):
        """Get specific region of target monitor"""
        try:
            # Calibration coordinates are already absolute,
            # we don't need to add monitor offset
            return {
                'left': self.capture_left,
                'top': self.capture_top,
                'width': self.capture_width,
